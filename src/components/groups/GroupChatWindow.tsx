...
export function GroupChatWindow({ group, currentUser, onClose }: GroupChatWindowProps) {
  ...
  const fetchMembers = async () => {
    ...
    if (data) {
      const memberProfiles = data.map((m) => m.profiles as unknown as Profile);
  ...
  return (
    <div className="fixed bottom-0 left-4 w-96 bg-card rounded-t-xl shadow-xl border z-50" dir="rtl">
      {/* Header */}
      <div className="flex items-center justify-between p-3 border-b bg-primary text-primary-foreground rounded-t-xl">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs opacity-80">{members.length} أعضاء</p>
          </div>
        </div>
      ...
      <Sheet open={showMembers} onOpenChange={setShowMembers}>
        <SheetContent side="right" dir="rtl">
          <SheetHeader>
            <SheetTitle>أعضاء المجموعة ({members.length})</SheetTitle>
          </SheetHeader>
