...
export default function ProfilePage() {
  ...
  const fetchProfile = async () => {
    ...
    if (isOwnProfile) {
      ...
      const { data } = await supabase.from('profiles_public').select('*').eq('id', id).single();
  ...
  const fetchPosts = async () => {
    if (!id) return;
    const { data, count } = await supabase.from('posts').select('*, profiles(*)', { count: 'exact' }).eq('user_id', id).order('created_at', { ascending: false });
    if (data) {
      setPosts(data as unknown as Post[]);
      setPostCount(count || 0);
      setPhotos(data.filter((p: any) => p.image_url).map((p: any) => p.image_url as string));
    }
  };
  ...
  const fetchFriends = async () => {
    ...
    const { data, count } = await supabase
      .from('friendships')
      .select(`requester_id, addressee_id, requester:profiles!friendships_requester_id_fkey(id, username, full_name, avatar_url, is_online), addressee:profiles!friendships_addressee_id_fkey(id, username, full_name, avatar_url, is_online)`, { count: 'exact' })
      .eq('status', 'accepted')
      .or(`requester_id.eq.${id},addressee_id.eq.${id}`);
    if (data) {
      setFriends(data.map(f => (f.requester_id === id ? f.addressee : f.requester) as unknown as Profile));
  ...
  const fetchSavedPosts = async () => {
    if (!user) return;
    const { data } = await supabase.from('saved_posts').select(`*, post:posts(*, profiles(*))`).eq('user_id', user.id).order('created_at', { ascending: false });
    if (data) setSavedPosts(data as unknown as SavedPost[]);
  };

  const fetchPinnedPosts = async () => {
    if (!id) return;
    const { data } = await supabase.from('pinned_posts' as any).select('post_id').eq('user_id', id);
    if (data) setPinnedPostIds(new Set((data as any[]).map(p => p.post_id)));
  };

  const checkHasStory = async () => {
    if (!id) return;
    const { count } = await supabase.from('stories').select('*', { count: 'exact', head: true }).eq('user_id', id).gt('expires_at', new Date().toISOString());
    setHasStory((count || 0) > 0);
  };

  const checkFriendship = async () => {
    if (!id || !user || id === user.id) return;
    const { data } = await supabase.from('friendships').select('*').or(`and(requester_id.eq.${user.id},addressee_id.eq.${id}),and(requester_id.eq.${id},addressee_id.eq.${user.id})`).single();
    if (data) {
  ...
  const sendFriendRequest = async () => {
    ...
    const { data, error } = await supabase.from('friendships').insert({ requester_id: user.id, addressee_id: id }).select().single();
    if (!error && data) {
      await supabase.rpc('create_notification', {
        p_user_id: id, p_type: 'friend_request',
        p_content: `${currentUserProfile.full_name || currentUserProfile.username} ${t('friends.sentFriendRequest')}`,
  ...
  const handleFriendRequest = async (accept: boolean) => {
    ...
    if (!error && friendship) {
      await supabase.rpc('create_notification', {
        ...
        p_content: `${currentUserProfile.full_name || currentUserProfile.username} ${accept ? t('friends.acceptedRequest') : t('friends.rejectedRequest')}`,
  ...
  const handleSaveLink = async () => {
    ...
    toast({ title: t('profile.linkSaved') });
  ...
  const handleToggleActivityStatus = async () => {
    ...
    toast({ title: newStatus ? t('profile.activityVisible') : t('profile.activityHidden') });
  };
        ...
        >
          ...
          {isOwnProfile && (
            <>
              ...
              <Button variant="secondary" size="sm" className={`absolute bottom-4 ${isRTL ? 'left-4' : 'right-4'} rounded-xl glass border-0`}
                onClick={(e) => { e.stopPropagation(); coverInputRef.current?.click(); }} disabled={uploading}>
                {uploading ? <Loader2 className={`h-4 w-4 animate-spin ${isRTL ? 'ml-2' : 'mr-2'}`} /> : <Camera className={`h-4 w-4 ${isRTL ? 'ml-2' : 'mr-2'}`} />}
        ...
        <div className="px-4 md:px-8">
            ...
            <Dialog open={showAvatarOptions} onOpenChange={setShowAvatarOptions}>
              <DialogTrigger asChild>
                <div className={`relative cursor-pointer group ${hasStory ? 'ring-4 ring-primary ring-offset-2 ring-offset-background rounded-full' : ''}`}>
                  ...
                  {!isOwnProfile && (
                    <div className={`absolute bottom-4 ${isRTL ? 'right-0' : 'left-0'}`}>
                      <OnlineIndicator isOnline={(profile as any).is_online} showDot={true} className="w-4 h-4" />
                    </div>
                  )}
                  {isOwnProfile && (
                    <Button variant="secondary" size="icon" className={`absolute bottom-2 ${isRTL ? 'left-2' : 'right-2'} rounded-full opacity-0 group-hover:opacity-100 transition-opacity`}>
                      <Camera className="h-4 w-4" />
                    </Button>
                  )}
                </div>
              </DialogTrigger>
              <DialogContent className="sm:max-w-xs" dir={isRTL ? 'rtl' : 'ltr'}>
            ...
            <div className="flex-1">
              ...
              {isOwnProfile && editingLink && (
                <div className="flex items-center gap-2 mt-2">
                  ...
                  <Button size="sm" onClick={handleSaveLink}>{t('profile.save')}</Button>
                  <Button size="sm" variant="ghost" onClick={() => setEditingLink(false)}>{t('profile.cancel')}</Button>
                </div>
              )}
              {isOwnProfile && !editingLink && (
                <Button variant="ghost" size="sm" className="mt-1 h-7 text-xs gap-1" onClick={() => setEditingLink(true)}>
                  <Edit3 className="h-3 w-3" />
                  {(profile as any).link_url ? t('profile.editLink') : t('profile.addLink')}
                </Button>
              )}
              ...
              <div className="flex items-center gap-5 mt-4">
                <div className="glass rounded-xl px-4 py-2 text-center">
                  ...
                  <span className="text-xs text-muted-foreground">{t('profile.friendStat')}</span>
                </div>
                <div className="glass rounded-xl px-4 py-2 text-center">
                  <span className="font-bold text-lg block">{postCount}</span>
                  <span className="text-xs text-muted-foreground">{t('profile.postStat')}</span>
                </div>
              </div>
              ...
              {isOwnProfile && (
                <div className="flex items-center gap-2 mt-3">
                  ...
                  <span className="text-sm text-muted-foreground">{t('profile.showActivity')}</span>
                </div>
              )}
            </div>
              ...
              {!isOwnProfile && (
                <>
                  ...
                  {friendshipStatus === 'pending' && (
                    <Button variant="secondary" disabled>{t('profile.requestSent')}</Button>
                  )}
                  {friendshipStatus === 'received' && (
                    <div className="flex gap-2">
                      <Button onClick={() => handleFriendRequest(true)}>
                        <Check className={`h-4 w-4 ${isRTL ? 'ml-2' : 'mr-2'}`} />{t('friends.accept')}
                      </Button>
                      <Button variant="outline" onClick={() => handleFriendRequest(false)}>
                        <X className={`h-4 w-4 ${isRTL ? 'ml-2' : 'mr-2'}`} />{t('friends.reject')}
                      </Button>
                    </div>
                  )}
                  {friendshipStatus === 'accepted' && (
                    <>
                      <Button variant="secondary" onClick={handleMessage}>
                        <MessageCircle className={`h-4 w-4 ${isRTL ? 'ml-2' : 'mr-2'}`} />{t('profile.sendMessage')}
                      </Button>
                      <Button variant="outline" onClick={removeFriend}>
                        <UserMinus className={`h-4 w-4 ${isRTL ? 'ml-2' : 'mr-2'}`} />{t('profile.removeFriend')}
                      </Button>
                    </>
                  )}
                </>
          ...
          <Tabs defaultValue="posts" className="mt-8">
            <TabsList className="w-full justify-start flex-wrap bg-muted/60 rounded-xl h-11 gap-0">
              <TabsTrigger value="posts" className="rounded-lg font-semibold data-[state=active]:shadow-card">{t('profile.posts')}</TabsTrigger>
              <TabsTrigger value="about" className="rounded-lg font-semibold data-[state=active]:shadow-card">{t('profile.about')}</TabsTrigger>
              <TabsTrigger value="friends" className="rounded-lg font-semibold data-[state=active]:shadow-card">{t('profile.friends')}</TabsTrigger>
              <TabsTrigger value="photos" className="rounded-lg font-semibold data-[state=active]:shadow-card">{t('profile.photos')}</TabsTrigger>
              {isOwnProfile && (
                  ...
                  <div key={post.id} className="relative">
                    ...
                    {isOwnProfile && (
                      <Button
                        variant="ghost"
                        size="icon"
                        className={`absolute top-2 ${isRTL ? 'left-2' : 'right-2'} h-8 w-8`}
                        onClick={() => handleTogglePinPost(post.id)}
                        title={pinnedPostIds.has(post.id) ? t('feed.unpinPost') : t('feed.pinPost')}
                      >
            ...
            <TabsContent value="about" className="mt-5">
              <Card className="glass rounded-2xl p-6 border-border/50">
                <h3 className="font-semibold mb-5 text-lg">{t('profile.generalInfo')}</h3>
                <div className="space-y-4 text-sm">
                  <div className="flex items-center gap-3 p-3 bg-muted/40 rounded-xl">
                    <User className="h-4 w-4 text-muted-foreground flex-shrink-0" />
                    <div><span className="text-muted-foreground text-xs block">{t('profile.usernameLabel')}</span><span className="font-medium">@{profile.username}</span></div>
                  </div>
                  {profile.full_name && (
                    <div className="flex items-center gap-3 p-3 bg-muted/40 rounded-xl">
                      <Users className="h-4 w-4 text-muted-foreground flex-shrink-0" />
                      <div><span className="text-muted-foreground text-xs block">{t('profile.fullNameLabel')}</span><span className="font-medium">{profile.full_name}</span></div>
                    </div>
                  )}
                  {profile.bio && (
                    <div className="p-3 bg-muted/40 rounded-xl">
                      <span className="text-muted-foreground text-xs block mb-1">{t('profile.bioLabel')}</span>
                  ...
                  {(profile as any).link_url && (
                    <div className="flex items-center gap-3 p-3 bg-muted/40 rounded-xl">
                      ...
                      <div><span className="text-muted-foreground text-xs block">{t('profile.linkLabel')}</span>
                  ...
                  <div className="flex items-center gap-3 p-3 bg-muted/40 rounded-xl">
                    ...
                    <div><span className="text-muted-foreground text-xs block">{t('profile.joinDate')}</span><span className="font-medium">{new Date(profile.created_at).toLocaleDateString(isRTL ? 'ar-EG' : 'en-US')}</span></div>
                  </div>
                </div>
              </Card>
            </TabsContent>
            ...
            <TabsContent value="friends" className="mt-5">
              {friends.length === 0 ? (
                <Card className="glass rounded-2xl p-12 text-center border-border/50">
                  ...
                  <p className="text-muted-foreground font-medium">{t('profile.noFriends')}</p>
                </Card>
              ) : (
            ...
            <TabsContent value="photos" className="mt-5">
              {photos.length === 0 ? (
                <Card className="glass rounded-2xl p-12 text-center border-border/50">
                  <ImageIcon className="h-12 w-12 mx-auto mb-3 text-muted-foreground/30" />
                  <p className="text-muted-foreground font-medium">{t('profile.noPhotos')}</p>
                </Card>
              ) : (
                <>
              ...
              <TabsContent value="saved" className="mt-5 space-y-4">
                {savedPosts.length === 0 ? (
                  <Card className="glass rounded-2xl p-12 text-center border-border/50">
                    ...
                    <p className="text-muted-foreground font-medium">{t('profile.noSavedPosts')}</p>
                    <p className="text-sm text-muted-foreground mt-1">{t('profile.savePosts')}</p>
                  </Card>
                ) : (
